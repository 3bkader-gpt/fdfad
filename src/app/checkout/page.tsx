...
export default function CheckoutPage() {
  ...
  return (
    <main className="min-h-screen bg-[#FAFAFA] text-[#2C3E35] pb-12">
      <nav className="flex items-center justify-between bg-white px-6 py-4 border-b border-[#2C3E35]/5">
        <Link href="/" className="rounded-full bg-[#FAFAFA] p-2 transition-colors hover:bg-zinc-100">
      ...
      <div className="mx-auto max-w-md px-6 pt-8">
        <div className="mb-8 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-black/5 text-left">
          <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-40 mb-4">Order Summary</h2>
          <div className="flex flex-col gap-3">
            {items.map((item) => (
               <div key={item.product.id} className="flex justify-between text-sm">
                  <span className="opacity-70 truncate pr-4">{item.quantity}x {item.product.title}</span>
                  <span className="font-medium whitespace-nowrap">{item.product.price * item.quantity} EGP</span>
               </div>
            ))}
            <div className="mt-2 pt-4 border-t border-[#2C3E35]/5 flex justify-between items-center text-left">
        ...
        <form onSubmit={handleSubmit(handleCheckout)} className="flex flex-col gap-6 text-left">
           <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-40">Delivery Details</h2>
           
           <div className="flex flex-col gap-1.5">
             <label className="text-[10px] font-bold uppercase tracking-widest opacity-60">Full Name</label>
             <input 
               {...register('fullName')}
               className={`rounded-xl bg-white px-4 py-3.5 text-sm shadow-sm ring-1 transition-all focus:outline-none focus:ring-2 ${errors.fullName ? 'ring-red-200 focus:ring-red-100' : 'ring-black/5 focus:ring-[#C89B7E]/30'}`}
           ...
           <div className="flex flex-col gap-1.5">
             <label className="text-[10px] font-bold uppercase tracking-widest opacity-60">Mobile Number</label>
             <input 
               {...register('phone')}
               className={`rounded-xl bg-white px-4 py-3.5 text-sm shadow-sm ring-1 transition-all focus:outline-none focus:ring-2 ${errors.phone ? 'ring-red-200 focus:ring-red-100' : 'ring-black/5 focus:ring-[#C89B7E]/30'}`}
           ...
           <div className="flex flex-col gap-1.5">
             <label className="text-[10px] font-bold uppercase tracking-widest opacity-60">Governorate</label>
             <select 
               {...register('governorate')}
               className={`rounded-xl bg-white px-4 py-3.5 text-sm shadow-sm ring-1 transition-all focus:outline-none focus:ring-2 ${errors.governorate ? 'ring-red-200 focus:ring-red-100' : 'ring-black/5 focus:ring-[#C89B7E]/30'}`}
             >
               <option value="">Select Region</option>
               {GOVERNORATES.map(g => <option key={g} value={g}>{g}</option>)}
             </select>
             {errors.governorate && <p className="text-[10px] text-red-500 font-medium">{errors.governorate.message}</p>}
           </div>

           <div className="flex flex-col gap-1.5">
             <label className="text-[10px] font-bold uppercase tracking-widest opacity-60">Detailed Address</label>
             <textarea 
               {...register('address')}
               rows={3}
               className={`rounded-xl bg-white px-4 py-3.5 text-sm shadow-sm ring-1 transition-all focus:outline-none focus:ring-2 ${errors.address ? 'ring-red-200 focus:ring-red-100' : 'ring-black/5 focus:ring-[#C89B7E]/30'}`}
           ...
           <div className="flex flex-col gap-1.5">
             ...
             <input 
               {...register('notes')}
               className="rounded-xl bg-white px-4 py-3.5 text-sm shadow-sm ring-1 ring-black/5 focus:outline-none focus:ring-2 focus:ring-[#C89B7E]/30 transition-all"
